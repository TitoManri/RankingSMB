...
switch ($op) {
    ...
    case 'SubirLibro': {
            ...
            if (
                !$ID_Libro || !$Titulo || !$Publicante || !$Autor || !$Lanzamiento || !$Publico
                || !$Sinopsis || !$Poster || !$TotalPaginas || !$LinkCompraGoogle
            ) {
                $response = ["exito" => false, "msg" => "Hubo un error al conseguir la información del libro."];
                echo json_encode($response);
                exit;
            ...
            try {
                ...
                if ($confirmarInsertado) {
                    $response = ["exito" => true, "msg" => "Se subió el libro por primera vez."];
                } else {
